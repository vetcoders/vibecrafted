...
def _seal_runtime_pack_for_admission(payload: Path) -> Path:
    ...
    terminal_src = (
        Path(installer.__file__).resolve().parent / "vc-terminal-product-entry.sh"
    )
...
def _plan(payload: Path, capsys, **extra) -> dict:
    code = installer.cmd_runtime_install(_ns(payload, rescue=True, plan=True, **extra))
...
def test_plan_inventories_missing_history_and_live_damage_without_writes(
    ...
    assert not any((paths["runtime_home"] / ".installer-backups" / "rescue").rglob("*"))


def test_rescue_preserves_user_config_and_foreign_commands(tmp_path, installed, capsys):
    paths, payload, _ = installed
...
def test_interrupted_publish_then_retry_recovers(
    ...
    assert (
        installer._sha256_bytes(archive.read_bytes())
        == first["archived_receipt"]["sha256"]
    )
...
def test_target_installer_without_rescue_is_honest_bootstrap(
    tmp_path, installed, capsys
):
    paths, _, _ = installed
    _plant_missing_historical(paths)
...
def test_target_identity_binds_payload_bytes_not_version_hash(
    tmp_path, installed, capsys
):
    paths, payload, _ = installed
    _plant_missing_historical(paths)
...
def test_verify_destination_refuses_wrapper_skill_config_and_shell_drift(
    ...
    wrapper.unlink()
    wrapper.symlink_to(
        paths["runtime_home"] / "tools/vibecrafted-current/.venv/bin/vc-status"
    )
...
def test_classify_records_symlink_target_and_directory_listing_for_binding(
    ...
    _, plan = _plan(payload, capsys)
    drifted = next(entry for entry in plan["ownership"] if entry["path"] == str(owned))
    ...
    foreign_dir = next(
        entry for entry in plan["ownership"] if entry["path"] == str(occupied)
    )
...
def test_snapshot_covers_new_publication_and_validates_evidence(
    ...
    new_skill = (
        newer / "vibecrafted-core/vibecrafted_core/skills/vc-rescue-probe/SKILL.md"
    )
...
def test_same_version_added_skill_refuses_without_publication(installed, capsys):
    ...
    _plant_missing_historical(paths)
    new_skill = (
        payload / "vibecrafted-core/vibecrafted_core/skills/vc-rescue-probe/SKILL.md"
    )
    new_skill.parent.mkdir(parents=True, exist_ok=True)
    new_skill.write_text("# vc-rescue-probe\n")
    _seal_runtime_pack_for_admission(payload)
...
def test_fix_rc_is_explicit_planned_stanza_preserving_user_content(installed, capsys):
    paths, payload, _ = installed
    _plant_missing_historical(paths)
    zshrc = Path.home() / ".zshrc"
    ...
    assert code == 0
    stanza = next(
        item for item in plan["shell"]["stanzas"] if item["path"] == str(zshrc)
    )
...
def test_pack_admission_uses_canonical_provenance_contract(tmp_path):
    ...
    (payload2 / "runtime-pack-provenance.json").write_text(
...
def test_same_type_foreign_command_replacement_refuses(installed, capsys):
    ...
    _code, plan = _plan(payload, capsys)
...
def test_interrupted_resume_changed_mode_refuses(installed, capsys, monkeypatch):
    paths, payload, _ = installed
    _plant_missing_historical(paths)
    _, plan = _plan(payload, capsys)
...
def test_interrupted_resume_valid_same_binding_resumes(installed, capsys, monkeypatch):
    paths, payload, _ = installed
    planted = _plant_missing_historical(paths)
    _, plan = _plan(payload, capsys)
    original = installer._publish_runtime_config_transaction
...
def test_rollback_corrupted_label_reports_residual(tmp_path, roots):
    ...
    assert any(
        "malformed" in item["reason"] or "mismatched" in item["reason"]
        for item in residuals
    )
    assert marker.read_text(encoding="utf-8") == "stay\n"
...
def test_rollback_corrupt_copied_bytes_reports_residual(installed, tmp_path):
    ...
    assert residuals
    assert any(
        "mismatched" in item["reason"] or "evidence" in item["reason"]
        for item in residuals
    )
...
def test_snapshot_refuses_uncapturable_physical_path(installed, tmp_path, monkeypatch):
    paths, payload, _ = installed
    receipt = _load_receipt(paths)
...
def test_preference_conflict_keeps_pending_recoverable(installed, capsys, monkeypatch):
    paths, payload, _ = installed
    _plant_missing_historical(paths)
    _, plan = _plan(payload, capsys)
...
def test_verify_destination_refuses_unrelated_pending_identity(installed, capsys):
    paths, payload, _ = installed
    _plant_missing_historical(paths)
    _, plan = _plan(payload, capsys)
    code, _result = _apply(payload, capsys, plan["plan_digest"])
    assert code == 0
...
def test_owned_pending_identity_requires_validated_binding():
    ...
    assert (
        installer._runtime_rescue_pending_matches_owned_identity(stub, expected)
        is False
    )
    assert (
...
def test_prepublication_failure_preserves_post_rescue_user_file(
    ...
    def fail_before_publication(**kwargs):
        raise RuntimeError(
            "W2 injected planning failure before new capture/publication"
        )

    monkeypatch.setattr(
        installer, "_build_runtime_rescue_plan", fail_before_publication
    )
    code, _second = _apply(payload, capsys, "a" * 64)
    assert code == 2
...
def test_product_owned_shell_check_does_not_execute_user_startup(installed, tmp_path):
