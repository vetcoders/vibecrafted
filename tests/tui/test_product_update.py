...
def test_product_update_source_contract() -> None:
    ...
    release = (REPO_ROOT / "scripts/build-vibecrafted-release.sh").read_text(
        encoding="utf-8"
    )
    ...
    authored = (REPO_ROOT / "tests/tui/test_product_update.py").read_text(
        encoding="utf-8"
    )
    ...
    assert launch.index("adoptPendingProductUpdateIfNeeded") < launch.index(
        "connectCommandDeck"
    )
    recover = delegate[delegate.index("beginProductUpdateRecover") :]
...
def test_product_update_quit_stays_ui_only() -> None:
    ...
    assert (
        "nothing here stops frame" in coordinator.lower()
        or "Frame, terminals, agents" in coordinator
    )
...
def test_product_update_helper_refuses_harness_flags_in_production(
    tmp_path: Path,
) -> None:
    dest = tmp_path / "Installed.app"
    dest.mkdir()
    ...
    result = _run_helper(
        ...
        env={
            ...
            if k != "VIBECRAFTED_UPDATE_HELPER_HARNESS"
        },
    )
...
def _run_helper(
    args: list[str], env: dict[str, str] | None = None, timeout: float = 20
):
    HELPER.chmod(HELPER.stat().st_mode | stat.S_IXUSR)
...
def _isolated_product_env(
    ...
    sockets = (
        frame_socket_dir
        if frame_socket_dir is not None
        else tmp_path / "isolated-frame-sockets"
    )
...
def _assert_isolated_receipt_roots(receipt: dict[str, object], tmp_path: Path) -> None:
    ...
    for name, value in roots.items():
        ...
        assert resolved.startswith(prefix), (
            f"receipt root {name} escaped isolation: {resolved}"
        )
...
def _assert_receipt_names_live_app(receipt: dict[str, object], dest: Path) -> None:
    ...
    for name, value in roots.items():
        resolved = str(Path(str(value)).resolve())
        assert ".vc-update-capture-" not in resolved, (
            f"receipt root {name} names the capture"
        )
        assert not resolved.endswith("/prior.app"), (
            f"receipt root {name} still names prior.app"
        )
...
def test_product_update_helper_copy_failure_keeps_destination(tmp_path: Path) -> None:
    with _SignedApps(tmp_path) as apps:
        ...
        journal = json.loads(
            Path(str(receipt) + ".journal.json").read_text(encoding="utf-8")
        )
...
def test_product_update_result_before_new_ui_and_restore(tmp_path: Path) -> None:
    with _SignedApps(tmp_path) as apps:
        ...
        try:
            ...
            assert (
                json.loads(restore2.read_text(encoding="utf-8"))["detail"] == "restored"
            )
            assert _identity_token(dest) == apps.prior_identity
            assert _identity_token(prior2) == apps.prior_identity
...
class _IsolatedFrameSession:
    ...
    def _wait_worker(self) -> None:
        ...
        if (
            ...
            raise AssertionError(
                "Frame PTY worker did not publish its process identity"
            )
    ...
    def roundtrip(self, command: str) -> str:
        ...
        typed = self._action(
            "write-chars", "--pane-id", self.pane_id, f"{nonce} {command}"
        )
    ...
    def assert_survived(self) -> None:
        ...
        assert (
            self.worker_tty.endswith(tty_now) or Path(self.worker_tty).name == tty_now
        )
    ...
    try:
        ...
        with _SignedApps(tmp_path) as apps:
            ...
            assert not evidence_path.exists(), (
                "caller-written pack enum is not installer proof"
            )
