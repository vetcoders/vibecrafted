...
| --------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| workflow, implement, review, research, marbles                                                                                          | public deck -> core CLI -> workflow; `--session` on a single-agent launcher is `manual_resume_session` (same path as resume --session). Research refuses `--session`. | shared core flags; session continuation stays in the selected checkout | strict selection, private core input                                      | focused source tests; live agy High `--conversation` resume is source-probed, not an installed-generation claim                      |
...
| Agy           | `--model` after the binary                             | stream-json stdin (`--print=` + `--input-format stream-json`) | source tested; live `--conversation` resume probed 2026-09-11 on gemini-3.8-flash-high; native fork unsupported; installed generation is a separate admission |
...
Resume, fork, and single-agent workflow launchers share `--session <session_id|current|last>`. `--run-id` is a
separate control-plane identity and cannot be combined with `--session`.
On a workflow launcher, `--session` continues the named provider session through
`manual_resume_session`; it does not start a fresh session and is not a run id,
PID, or workspace id. Research refuses `--session` because it is a multi-agent swarm.
...
that their product can never fork. Headless exact-session resume for Agy uses
`--conversation` composed onto the private stream-json stdin lane (agy 1.2.1
probed 2026-09-11). Interactive exact-session resume uses Agy's
`--conversation`, Junie's `--resume --session-id`, Cursor's `--resume`, and Grok's
`--resume`. Junie and Cursor headless resume remain unverified. These argv contracts have help evidence; their real live-source
concurrency is not certified by an installed generation.
