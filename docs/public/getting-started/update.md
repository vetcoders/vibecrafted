...
Install and update also reconcile **skill-copy shadows**: real directory copies of bundled skills that a pre-3.x installer left in per-runtime skill dirs such as `~/.junie/skills`. A copy whose Vibecrafted provenance is proven is moved into `~/.vibecrafted/backups/installer/shadowed-views-<timestamp>/` and then removed, so the canonical `~/.agents/skills` view is the only truth. Provenance is proven from content only: an identical tree, an anchored Vibecrafted marker shared with the store copy, or a `SKILL.md` hash listed in the release manifest (`SKILL_PROVENANCE.json`) that ships inside the skill store — which is what identifies copies left by the June-2026 installers, since they predate every marker. A `vc-*` directory whose provenance cannot be proven is only reported — never removed, and neither is anything reached through a symlinked skill directory. `vibecrafted doctor` names both cases; see [Doctor](../troubleshooting/doctor.md).
