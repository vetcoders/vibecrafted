...
The view writer never removes a real directory. It replaces a symlink, a junction or a stray file with the correct link, and leaves anything else where it is with a `Keeping real directory …` line. By the time it runs, every copy whose provenance could be proven has already been quarantined, so what is left is a directory nobody could vouch for — and overwriting that with a symlink would delete an operator's own skill with no backup. On a first-ever install the canonical `~/.agents/skills/<skill>` view does not exist yet, so reconciliation defers: copies are reported that run and reconciled on the next one.
