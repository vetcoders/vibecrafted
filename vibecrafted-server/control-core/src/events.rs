...
fn open_archive_child_no_follow(archive_dir: &Path, name: &Path) -> io::Result<File> {
    // Windows limitation: no openat(O_NOFOLLOW). Open by joined path after
    // rejecting a last-component symlink via symlink_metadata.
    let child = archive_dir.join(name);
    let meta = fs::symlink_metadata(&child)?;
    if meta.file_type().is_symlink() {
        return Err(io::Error::new(
            io::ErrorKind::PermissionDenied,
            "event archive entry is a symlink",
        ));
    }
