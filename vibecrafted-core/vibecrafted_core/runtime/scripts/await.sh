...
usage() {
  cat <<'EOF_USAGE'
Usage: await.sh [claude|codex|agy|junie|grok|cursor|kimi] [--last] [--run-id <id>] [--research] [--auto-synthesize] [--describe] [--interval <sec>] [--timeout <sec>] [--startup-grace <sec>] [targets...]
...
def usage() -> None:
    print(
        "Usage: await.sh [claude|codex|agy|junie|grok|cursor|kimi] [--last] [--run-id <id>] "
...
while i < len(argv):
    arg = argv[i]
    if arg in {"claude", "codex", "agy", "junie", "grok", "cursor", "kimi"} and not agent:
        agent = arg
    elif arg == "--last":
        use_last = True
    elif arg == "--describe":
