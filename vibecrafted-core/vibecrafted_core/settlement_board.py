...
def _running_session_names(output: str) -> tuple[str, ...]:
    return tuple(name for name, _age in _running_sessions(output))


def _discover_running_sessions_with_age(
    env: Mapping[str, str] | None = None,
) -> tuple[tuple[str, int | None], ...] | None:
    try:
        from .settlement_history import _get_vc_frame_socket_dir
        ...
        for entry in os.scandir(sock_dir):
            ...
            try:
                ...
                if stat.S_ISSOCK(st.st_mode) or stat.S_ISREG(st.st_mode):
                    age = max(0, int(now - st.st_ctime))
                    sessions.append((entry.name, age))
            except OSError:
                continue
        return tuple(sorted(sessions, key=lambda s: s[0]))
...
class SettlementBoardPublisher:
    ...
    def __init__(
        ...
        *,
        ...
        self._delivered_payload_by_session = self._delivered_payloads
    ...
    def _deliver(self, payload: str) -> DeliveryReport:
        ...
        discovered: tuple[tuple[str, int | None], ...]
        if self.runner is not _default_runner:
            ...
            socket_discovered = _discover_running_sessions_with_age(self.env)
            if socket_discovered is not None:
                discovered = tuple(
                    (session, age)
                    for session, age in socket_discovered
                    if session not in _NON_PLUGIN_SESSION_NAMES
                )
            else:
                try:
