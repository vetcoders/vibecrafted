...
- **Dogfood (isolated `AICX_HOME`, zero operator-state mutation):** catalog rebuild admits **21 cursor sessions**; `sessions list --agent cursor` renders them with parsed timestamps; `continuity show -p host-a -H 96` lists session `004ffd2e-…` (this very conversation) as `open: cursor` with its intents/decisions; `extract cursor --file` renders wrapper-free operator speech
